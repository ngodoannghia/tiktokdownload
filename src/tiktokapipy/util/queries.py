...
async def get_video_detail_async(video_id: int, context: AsyncContext):
    ...
    response = await make_request_async("item/detail/", 0, video_id, context)
    print("==== Time make request video: %s second ==== " % (time.time() - start_time))

    itemInfo = response.get("itemInfo")
    if itemInfo is not None:
        itemStruct = itemInfo.get("itemStruct")
        if itemStruct is not None:
            item_video = itemStruct.get("video")
            if item_video is not None:
                subtitleInfors = item_video.get("subtitleInfos")
                if subtitleInfors is not None:
                    for i in range(len(subtitleInfors)):
                        if "Version" in subtitleInfors[i]:
                            response["itemInfo"]["itemStruct"]["video"]["subtitleInfos"][i]["Version"] = 1
