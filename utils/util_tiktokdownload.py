...
async def save_image_slideshow(url, id, i):
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            downloaded_avatar_larger =  io.BytesIO(await resp.read())
            with open(path.join(AppConfig.directory_tmp, f"temp_{id}_{i:02}.jpeg"), 'wb') as f:
                f.write(downloaded_avatar_larger.getbuffer())
...
async def crawl_info(link):
    async with AsyncTikTokAPI() as api:
        ...
        os.makedirs(f"{AppConfig.root}/{id_user}/video/unwatermark", exist_ok=True)
        os.makedirs(f"{AppConfig.root}/{id_user}/audio",exist_ok=True)
        os.makedirs(f"{AppConfig.root}/{id_user}/avatar", exist_ok=True)
        os.makedirs(f"{AppConfig.root}/{id_user}/cover", exist_ok=True)
            
        key_video_unwatermark_normal = f"{AppConfig.root}/{id_user}/video/unwatermark/{id_video}.{format_video}"
        key_video_unwatermark_fullhd = f"{AppConfig.root}/{id_user}/video/unwatermark/{id_video}_fullhd.{format_video}"
        key_audio = f"{AppConfig.root}/{id_user}/audio/{id_music}.{format_audio}"
        key_avatar = f"{AppConfig.root}/{id_user}/avatar/{id_user}.{format_avatar}"
        key_cover = f"{AppConfig.root}/{id_user}/cover/{id_video}.{format_avatar}"
...
async def save_slideshow(video, image_post, dict_aws, url_avatar, url_cover, url_music):
    ...
    command = [
        "ffmpeg",
        "-r 2/5",
        f"-i {AppConfig.directory_tmp}/temp_{video.get('id')}_%02d.jpeg",
        f"-i {dict_aws.get('key_audio')}",
        f"-vf {vf}",
        "-c:v libx264",
        "-c:a aac",
    ...
    generated_files = glob.glob(path.join(AppConfig.directory_tmp, f"temp_{video.get('id')}*"))
...
async def download_video(link):
    ...
    response = {
        "video": {
            ...
            "link_unwatermark": key_video_unwatermark_normal.replace(AppConfig.root, "https://api2-download.ssstik.cx/static/tiktokdownload"),
            "link_unwatermark_fullhd": key_video_unwatermark_fullhd.replace(AppConfig.root, "https://api2-download.ssstik.cx/static/tiktokdownload"),
            "link_cover": key_cover.replace(AppConfig.root, "https://api2-download.ssstik.cx/static/tiktokdownload")
        },
        "user": {
            "id": user_info.get("id"),
            ...
            "link_avatar": key_avatar.replace(AppConfig.root, "https://api2-download.ssstik.cx/static/tiktokdownload")
        },
        "music": {
            ...
            "link_music": key_audio.replace(AppConfig.root, "https://api2-download.ssstik.cx/static/tiktokdownload")
        }
    }
